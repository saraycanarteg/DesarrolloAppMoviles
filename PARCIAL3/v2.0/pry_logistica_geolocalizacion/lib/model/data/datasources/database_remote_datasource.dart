...
class DatabaseRemoteDataSource {
  ...
  Future<void> actualizarUbicacion(String repartidorId, UbicacionModel ubicacion) async {
    // Guarda anti sesión cruzada: el GPS de este dispositivo solo puede
    // publicarse bajo el uid del usuario autenticado. Evita que la posición
    // de otro usuario (p. ej. el administrador) figure como la del repartidor.
    final uidSesion = FirebaseAuth.instance.currentUser?.uid;
    if (uidSesion != repartidorId) return;
  ...
  /// Lista en tiempo real de todos los repartidores (para el selector
  /// de asignación manual y el dashboard del administrador).
  Stream<List<RepartidorModel>> observarRepartidores() {
    return _database.ref().child(FirebaseConstants.repartidoresNode).onValue.map((event) {
      final data = event.snapshot.value as Map<dynamic, dynamic>?;
      if (data == null) return [];

      final lista = <RepartidorModel>[];
      data.forEach((key, value) {
        if (value is Map) {
          lista.add(RepartidorModel.fromMap(value, key.toString()));
        }
      });
  ...
  /// Edición (por el administrador) de los datos de contacto y del vehículo
  /// de un repartidor. El nombre y la cédula no se tocan: identifican a la
  /// persona y a la cuenta.
  Future<void> actualizarRepartidor(
    ...
    await _database.ref().child(FirebaseConstants.repartidoresNode).child(uid).update({
      'telefono': telefono,
      'email': email,
    ...
    // Best effort sobre usuarios/{uid}: las reglas pueden exigir
    // auth.uid == uid (como en el registro), en cuyo caso este update
    // falla para el admin y el perfil se queda con el dato anterior.
    try {
      await _database.ref().child(FirebaseConstants.usuariosNode).child(uid).update({
        'telefono': telefono,
        'email': email,
      });
    } catch (_) {}
  ...
  /// Elimina el registro de un repartidor (nodo `repartidores` y su última
  /// ubicación). La cuenta de Firebase Auth no puede borrarse desde el
  /// cliente, pero sin el nodo `repartidores` el login queda bloqueado
  /// ("Datos de repartidor no encontrados").
  Future<void> eliminarRepartidor(String uid) async {
    await _database.ref().child(FirebaseConstants.repartidoresNode).child(uid).remove();

    for (final nodo in [FirebaseConstants.ubicacionesNode, FirebaseConstants.usuariosNode]) {
      try {
        await _database.ref().child(nodo).child(uid).remove();
      } catch (_) {
        // Ignorar: las reglas pueden impedir borrar usuarios/{uid} al admin.
      }
    }
  }

  /// Stream en tiempo real de la última ubicación reportada por un repartidor.
  Stream<UbicacionModel?> observarUbicacion(String repartidorId) {
    return _database
        .ref()
        .child(FirebaseConstants.ubicacionesNode)
        .child(repartidorId)
      ...
      return UbicacionModel.fromMap(data);
    });
  }

  /// Guarda la ruta calculada bajo rutas/{pedidoId} para poder consultarla
  /// directamente sin necesidad de índices en las reglas de Firebase.
  Future<void> guardarRutaDePedido(RutaModel ruta) async {
    await _database
        .ref()
        .child(FirebaseConstants.rutasNode)
        .child(ruta.pedidoId)
        .set(ruta.toMap());
  }

  Future<RutaModel?> obtenerRutaDePedido(String pedidoId) async {
    ...
    return RutaModel.fromMap(snapshot.value as Map<dynamic, dynamic>);
  }

  /// El repartidor inicia el viaje hacia la tienda:
  /// pedido pasa de "asignado" a "en_ruta_a_tienda".
  Future<void> iniciarRuta(String pedidoId) async {
    await _database.ref().child(FirebaseConstants.pedidosNode).child(pedidoId).update({
      'estado': 'en_ruta_a_tienda',
      'enRutaEn': ServerValue.timestamp,
    });
  }

  /// El repartidor confirma que recogió el pedido en la tienda:
  /// pedido pasa a "recogido". (La validación de cercanía a la tienda
  /// se hace en el ViewModel con la última posición GPS.)
  Future<void> marcarRecogido(String pedidoId) async {
    await _database.ref().child(FirebaseConstants.pedidosNode).child(pedidoId).update({
      'estado': 'recogido',
      'recogidoEn': ServerValue.timestamp,
    });
  }

  /// El repartidor sale de la tienda hacia el cliente:
  /// pedido pasa a "en_ruta_a_cliente".
  Future<void> iniciarRutaCliente(String pedidoId) async {
    await _database.ref().child(FirebaseConstants.pedidosNode).child(pedidoId).update({
      'estado': 'en_ruta_a_cliente',
      'enRutaClienteEn': ServerValue.timestamp,
    });
  }

  /// El repartidor confirma la entrega: pedido pasa a "entregado"
  /// y el repartidor queda disponible nuevamente.
  Future<void> marcarEntregado(String pedidoId, String repartidorId) async {
    await _database.ref().child(FirebaseConstants.pedidosNode).child(pedidoId).update({
      'estado': 'entregado',
      'entregadoEn': ServerValue.timestamp,
    });
    ...
    return _database.ref().child(FirebaseConstants.pedidosNode).onValue.map((event) {
      ...
      data.forEach((key, value) {
        ...
        if (mapValue['repartidorId'] == repartidorId) {
          ...
          if (p.estado == EstadoPedido.esperandoConfirmacion || p.enCurso) {
            pedidoEncontrado = p;
          }
        }
      });
